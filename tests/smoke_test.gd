...
## Record an assertion, report its message on failure, and let remaining checks continue.
...
## Exercise live gameplay systems, input, transitions, and restart; exit nonzero on failure.
## Direct fixed-step calls accelerate simulation; graphical-only assertions need a real renderer.
func run() -> void:
	...
	check(
		game.mode == "title" and game.renderer.chunks.size() == 64,
		"Title and 64 terrain chunks initialize"
	)
	check(
		game.scenery.towers.size() == 16 and game.scenery.tree_props.size() > 300,
		"Scenery and radar towers initialize"
	)
	check(game.hud.radar != null, "Radar generated")
	game.start()
	check(game.mode == "playing" and game.enemies.size() == 3, "Title starts first wave")
	if DisplayServer.get_name() != "headless":
		check(Input.mouse_mode == Input.MOUSE_MODE_CAPTURED, "Starting captures mouse")
	...
	for i in range(30):
		p.step(1.0 / 60)
	check(
		absf(p.yaw) < deg_to_rad(1.3) and absf(p.tilt) < 0.001,
		"Small horizontal input produces only a small turn and no tilt"
	)
	p.mouse_motion(Vector2(1800, -1000))
	for i in range(60):
		p.step(1.0 / 60)
		check(
			absf(p.yaw_velocity) <= deg_to_rad(p.max_heading_rate) + 0.001,
			"Heading speed remains bounded on abrupt mouse input"
		)
	...
	for i in range(120):
		p.step(1.0 / 60)
	check(
		absf(rad_to_deg(p.yaw - held_heading)) < 0.1,
		"Stopping the mouse holds heading without ongoing turning"
	)
	p.mouse_motion(Vector2(-100, 0))
	for i in range(180):
		p.step(1.0 / 60)
	...
	p.position = Vector3(512, 200, 512)
	p.velocity = Vector3.ZERO
	Input.action_press("thrust")
	p.step(0.1)
	Input.action_release("thrust")
	check(p.velocity.y < 0, "Ceiling prevents main thrust acceleration")
	p.position = Vector3(512, 90, 512)
	p.velocity = Vector3.ZERO
	p.tilt = deg_to_rad(170)
	p.virtual_mouse_offset = Vector2(0, -240)
	Input.action_press("thrust")
	p.step(0.1)
	Input.action_release("thrust")
	...
	p.velocity = Vector3(-5, -9, 2)
	p.tilt = deg_to_rad(25)
	var assist_seen := false
	for i in range(600):
		p.step(1.0 / 60)
		assist_seen = assist_seen or p.landing_assist
		if p.landed or not p.alive:
			break
	check(
		assist_seen and p.alive and p.landed,
		"Pad assist guides an off-center tilted approach with empty fuel to a safe landing"
	)
	...
	p.velocity = Vector3(4, -6, 0)
	p.tilt = deg_to_rad(18)
	p.step(1.0 / 60)
	check(p.alive and p.landed, "Wider pad accepts moderate touchdown speed and tilt")
	p.respawn()
	p.landed = false
	p.position = game.terrain.home + Vector3(0, 12, 0)
	p.velocity = Vector3.DOWN * 4
	p.step(1.0 / 60)
	...
	motion.relative = Vector2(1000, -1000)
	game._unhandled_input(motion)
	check(
		is_equal_approx(p.virtual_mouse_offset.length(), p.virtual_mouse_radius),
		"Mouse motion accumulates and clamps its virtual radius"
	)
	p.respawn()
	...
	seed_enemy.target = seed_enemy.position
	seed_enemy.timer = 5
	seed_enemy.fire_timer = 0
	seed_enemy.step(0.1)
	check(has_projectile(3), "Seeder emits visible falling droplets")
	game.projectiles.clear()
	var shot_score: int = game.score.score
	game.projectiles.fire_cannon()
	check(
		game.score.score == shot_score - 1 and game.projectiles.shots.size() > 0,
		"Real cannon creates round and charges score"
	)
	game.projectiles.clear()
	# Real swept enemy collision, across the toroidal seam.
	var enemy: EnemyCraft = game.enemies[0]
	enemy.position = Vector3(1022, 50, 400)
	enemy.hp = 1
	...
	await process_frame
	p.position = Vector3(512, 70, 512)
	p.basis = Basis.IDENTITY
	enemy = game.enemies[0]
	enemy.position = Vector3(512, 70, 450)
	...
	enemy = game.enemies[0]
	enemy.position = p.position + Vector3(0, 0, -200)
	...
	check(has_enemy(3), "Later landscape spawns bombers")
	game.set_physics_process(false)
	for e in game.enemies:
		if e.kind == 3:
			e.fire_timer = 0
			e.step(0.05)
	check(has_projectile(4), "Bomber deposits visible virus packets")
	var fighter := EnemyCraft.new()
	game.add_child(fighter)
	fighter.setup(game, 1, game.player.position + Vector3(0, 30, -80), 71)
	fighter.fire_timer = 0
	fighter.step(0.05)
	check(has_projectile(1), "Fighter fires lethal projectile")
	fighter.dispose()
	var pest := EnemyCraft.new()
	game.add_child(pest)
	...
	if DisplayServer.get_name() != "headless":
		check(color.r > color.g, "Infection visibly recolors tree instances")
	var water_pos := Vector3.ZERO
	for i in range(16384):
		if game.terrain.land[i] == 0 and game.terrain.heights[i] < -3:
			water_pos = Vector3((i % 128) * 8 + 4, 0, (i / 128) * 8 + 4)
			break
	game.player.position = water_pos
	game.player.landed = false
	game.player.velocity = Vector3.DOWN * 10
	...
	game.start()
	check(
		game.mode == "playing" and game.score.lives == 3 and game.waves.landscape == 1,
		"Game over restarts without reopening"
	)
	...
	game._physics_process(0.02)
	check(
		game.mode == "gameover" and game.loss_reason == "LANDSCAPE LOST",
		"Nearly complete infection ends the campaign"
	)
...
## Return whether an active projectile has the requested projectile-system kind ID.
func has_projectile(kind: int) -> bool:
	for shot in game.projectiles.shots:
		if shot.kind == kind:
			return true
	return false

## Return whether the current wave includes the requested enemy role ID.
func has_enemy(kind: int) -> bool:
