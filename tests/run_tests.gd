...
## Run deterministic, non-rendering assertions for terrain, wrapping, controls, score, and progression.
## Count failures and return a failing process exit code if any assertion fails.
func _initialize() -> void:
	...
	check(
		WrapMath.canonical(Vector3(-3, 10, 1028)).is_equal_approx(Vector3(1021, 10, 4)),
		"Canonical coordinates"
	)
	var a := TerrainData.new()
	a.generate(1987, 1)
	var b := TerrainData.new()
	b.generate(1987, 1)
	...
	for t in range(0, 1024, 17):
		...
		check(
			absf(a.surface(Vector3(-0.001, 0, t)) - a.surface(Vector3(1023.999, 0, t))) < 0.001,
			"Wrapped surface query"
		)
	...
	check(
		is_equal_approx(rad_to_deg(previous_tilt), flight.max_declination),
		"Gentler control preserves full inversion range"
	)
	flight.requested_heading = 0
	flight.virtual_mouse_offset = Vector2.ZERO
	flight.mouse_motion(Vector2(100, -100))
	check(
		is_equal_approx(rad_to_deg(flight.requested_heading), -10),
		"Horizontal motion is proportional rather than an absolute compass jump"
	)
	...
	score.cannon()
	check(score.score == -1, "Cannon costs a point even at zero score")
	score.add(10001)
	check(
		score.lives == 5 and score.bombs == 4 and score.missiles == 4 and score.next_life == 15000,
		"Multiple extra-life thresholds"
	)
	score.add(-6000)
	score.add(6000)
	...
	for i in range(3):
		check(waves.advance() == "wave", "Four waves per landscape")
	check(waves.wave == 4 and waves.landscape == 1, "No early landscape advance")
	check(
		waves.advance() == "landscape" and waves.landscape == 2 and waves.wave == 1,
		"New landscape after fourth wave"
	)
	check(3 in waves.roster(), "Bombers introduced on landscape two")
	for i in range(7):
		waves.advance()
	check(
		waves.landscape == 3 and waves.wave == 4 and waves.advance() == "victory",
		"Campaign has a victory endpoint"
	)
