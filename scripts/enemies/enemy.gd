...
func setup(controller: Node3D, role: int, start: Vector3, seed_value: int) -> void:
 game=controller; kind=role; position=start
 rng.seed=seed_value
 hp=[2,3,1,5][kind]
 model=Models.craft(kind); add_child(model)
...
func choose_land() -> void:
 for attempt in range(32):
  target=WrapMath.canonical(position+Vector3(rng.randf_range(-100,100),0,rng.randf_range(-100,100)))
  if game.terrain.surface(target)>1: break
 target.y=game.terrain.ground(target)+10
func step(dt: float) -> void:
 if not alive: return
 age+=dt; timer-=dt; fire_timer-=dt
 ...
 match kind:
  ...
  2:
   var aim: Vector3 = to_player.normalized()*37+Vector3(sin(age*4)*9,cos(age*3)*5,cos(age*4)*9)
   velocity=velocity.lerp(aim,dt*2.2)
  3:
   velocity=Vector3(50,0,20*sin(age*0.15))
   velocity.y=clampf((105-position.y)*2,-15,15)
   if fire_timer<=0:
    game.projectiles.spawn(4,position,Vector3(velocity.x*0.4,-8,velocity.z*0.4)); fire_timer=1.0
 position+=velocity*dt
 position.y=maxf(position.y,game.terrain.ground(position)+4)
 if velocity.length()>0.5: look_at(position+velocity,Vector3.UP)
 ...
 if game.player.alive and WrapMath.delta(position,game.player.position).length()<3.2: game.player.die()
