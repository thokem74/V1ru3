## Procedural scenery, destructible prop records, spatial collision bins, and radar coverage.
class_name SceneryManager
extends Node3D
signal radar_tower_destroyed
var data: TerrainData
# Prop record: p=logical base position, kind=0 tree/1 building/2 tower, alive=hit state.
# Trees add scale/tree (MultiMesh index); other props add node (their visual root).
var props: Array[Dictionary] = []
# These dictionaries alias the matching props entries, so damage also changes coverage.
var towers: Array[Dictionary] = []
var trees: MultiMeshInstance3D
var tree_props: Array[int] = []
var home_node: Node3D
# Row-major 128x128 byte mask: 1 means at least one living tower covers the tile.
var covered := PackedByteArray()
# Vector2i bin -> array of shared prop records. Dead props stay indexed but are skipped.
var buckets: Dictionary = {}

## Generate deterministic trees, buildings, scanner towers, and the home pad.
## Build tree instances, spatial collision bins, and initial radar coverage once per landscape.
func setup(terrain: TerrainData) -> void:
	...
	for i in range(1100):
		...
		p.y = data.ground(p)
		if p.y < 1 or WrapMath.delta(p, data.home).length() < 35:
			continue
	...
	for i in range(42):
		var p := Vector3(rng.randf() * 1024, 0, rng.randf() * 1024)
		p.y = data.ground(p)
		if p.y < 1 or WrapMath.delta(p, data.home).length() < 40:
			continue
		var node := Node3D.new()
		add_child(node)
	...
	for z in range(4):
		for x in range(4):
			var p := Vector3(x * 256 + 128, 0, z * 256 + 128)
			p.y = data.ground(p)
			var node := Node3D.new()
			add_child(node)
			...
			towers.append(item)
			props.append(item)
	home_node = Node3D.new()
	add_child(home_node)
	Models.box(
		home_node,
		Vector3((TerrainData.PAD_HALF_SIZE + 1) * 2, 0.35, (TerrainData.PAD_HALF_SIZE + 1) * 2),
		Color("3c515c"),
		Vector3(0, 0.15, 0)
	)
	for x in [-TerrainData.PAD_HALF_SIZE, TerrainData.PAD_HALF_SIZE]:
		Models.box(home_node, Vector3(0.6, 0.1, 30), Color("8fe2cf"), Vector3(x, 0.4, 0))
	for z in [-TerrainData.PAD_HALF_SIZE, TerrainData.PAD_HALF_SIZE]:
		Models.box(home_node, Vector3(30, 0.1, 0.6), Color("8fe2cf"), Vector3(0, 0.4, z))
	for x in [-2, 2]:
		Models.box(home_node, Vector3(0.6, 0.1, 6), Color("f0de9e"), Vector3(x, 0.4, 0))
...
## Return the wrapped 32 m collision-bin coordinate for a world position.
func bucket(p: Vector3) -> Vector2i:
	return Vector2i(posmod(int(floor(p.x / 32)), 32), posmod(int(floor(p.z / 32)), 32))

## Recompute the tile coverage mask from living radar towers.
## Use planar wrapped distance so a tower covers nearby tiles across either map seam.
func rebuild_coverage() -> void:
	covered.resize(16384)
	covered.fill(0)
	for z in range(128):
		for x in range(128):
			var p := Vector3(x * 8, 0, z * 8)
			for tower in towers:
				var d := WrapMath.delta(p, tower.p)
				d.y = 0
				if tower.alive and d.length() < 190:
					covered[z * 128 + x] = 1
					break

## Move props to their nearest periodic image and update infected tree color/shape.
## Tree transforms are written into one MultiMesh instead of thousands of separate nodes.
func update_view(reference: Vector3) -> void:
	...
	for item in props:
		...
		if item.kind == 0:
			...
			var shape: Vector3 = Vector3(
				1.0 + infection * 0.5,
				1.0 - infection * 0.3,
				1.0 + infection * 0.5
			) * item.scale
			trees.multimesh.set_instance_transform(
				item.tree,
				Transform3D(Basis.IDENTITY.scaled(shape), p + Vector3(0, 3 * shape.y, 0))
			)
			trees.multimesh.set_instance_color(item.tree, Color("387150").lerp(Color("bc434b"), infection))
		else:
			item.node.position = p

## Test a swept projectile segment against nearby living props, using spatial bins first.
## Destroy the first matching prop and return its record, or an empty dictionary on a miss.
