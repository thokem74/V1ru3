## Chunked flat-shaded terrain. Infection alters colors, never the collision height field.
...
## Create the shared vertex-color material and the 64 chunk mesh instances.
## Call once per newly constructed renderer, after terrain data has been generated.
...
## Replace one 16-by-16 chunk mesh using current terrain heights and infection colors.
## Duplicate triangle vertices so each face has a flat normal and each tile its own color.
...
## Place each chunk in its nearest periodic image around reference.
## Rebuild at most two dirty chunks per call to spread infection rendering work across frames.
