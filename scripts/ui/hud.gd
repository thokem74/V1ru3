## Immediate-mode retro interface. UI functions read game state and draw in 1280-by-720 design coordinates.
class_name GameHUD
extends Control
...
# Cached terrain/coverage texture; actor markers remain immediate-mode overlays.
var radar: ImageTexture
var large_map := false

## Fill the window while allowing input to pass through to gameplay controls.
func _ready() -> void:
	mouse_filter = Control.MOUSE_FILTER_IGNORE
	set_anchors_and_offsets_preset(Control.PRESET_FULL_RECT)

## Draw one baseline-aligned text string using the shared font and design coordinates.
...
## Draw a translucent instrument/menu background with a one-pixel outline.
func panel(rect: Rect2) -> void:
	draw_rect(rect, PANEL)
	draw_rect(rect, Color("43616b"), false, 1)

## Regenerate the 128-by-128 terrain/coverage image and update its existing texture.
## The controller throttles this work; markers are drawn independently over the image.
func refresh_radar() -> void:
	if game.terrain == null or game.scenery == null:
		return
	...
	if radar == null:
		radar = ImageTexture.create_from_image(img)
	else:
		radar.update(img)

## Convert wrapped world X/Z to a pixel inside the supplied radar rectangle.
func map_point(p: Vector3, rect: Rect2) -> Vector2:
	return rect.position + Vector2(fposmod(p.x, 1024), fposmod(p.z, 1024)) / 1024 * rect.size

## Draw terrain, coverage, home, towers, detected enemies, and player heading.
## Only enemies under live radar coverage receive a marker.
func draw_radar(rect: Rect2) -> void:
	...
	if radar:
		draw_texture_rect(radar, rect, false)
	for i in range(1, 4):
		var offset := rect.size.x * i / 4
		draw_line(
			rect.position + Vector2(offset, 0),
			rect.position + Vector2(offset, rect.size.y),
			Color(0.6, 0.85, 0.8, 0.12)
		)
		draw_line(
			rect.position + Vector2(0, offset),
			rect.position + Vector2(rect.size.x, offset),
			Color(0.6, 0.85, 0.8, 0.12)
		)
	...
	for tower in game.scenery.towers:
		if tower.alive:
			draw_circle(map_point(tower.p, rect), 1.5, MUTED)
...
## Scale the fixed 1280-by-720 HUD design to the window and draw the current UI state.
func _draw() -> void:
	...
	draw_flight_hud()
	if large_map:
		draw_radar(Rect2(414, 138, 448, 448))
	if game.mode != "playing":
		modal()

## Draw flight instruments, weapon stocks, status text, aiming reticle, and target brackets.
## Project world markers through the camera, then convert back to HUD design coordinates.
func draw_flight_hud() -> void:
	...
	draw_rect(Rect2(44, 653, player.fuel * 1.7, 12), AQUA if player.fuel > 20 else Color("f17e68"))
	text(Vector2(222, 666), "%03d" % player.fuel, 18)
	text(Vector2(298, 642), "ALTITUDE", 12, MUTED)
	text(Vector2(298, 674), "%03dm" % player.altitude(), 24)
	text(Vector2(438, 642), "SPEED", 12, MUTED)
	text(Vector2(438, 674), "%02d m/s" % player.velocity.length(), 24)
	...
	var status := ""
	if player.landed:
		status = "REFUELING • THRUST TO LIFT OFF"
	elif player.landing_assist:
		status = "LANDING ASSIST • THRUST TO ABORT"
	elif player.position.y >= player.flight_ceiling:
		status = "THRUST CEILING"
	elif player.fuel < 20:
		status = "LOW FUEL • RETURN TO HOME"
	# Temporary event announcements take precedence over persistent flight status.
	...
	draw_arc(center, 50 * player.mouse_deadzone, 0, TAU, 16, AQUA, 1)
	draw_line(center - Vector2(56, 0), center + Vector2(56, 0), MUTED)
	draw_line(center - Vector2(0, 56), center + Vector2(0, 56), MUTED)
	draw_circle(center + player.virtual_mouse_offset / player.virtual_mouse_radius * 50, 4, GOLD)
	text(Vector2(1126, 576), "TILT %03d°" % rad_to_deg(player.tilt), 14)
	if player.alive:
		var nose := player.position - player.basis.z * 90
		...
		for enemy in game.enemies:
			var distance := WrapMath.delta(player.position, enemy.position).length()
			if distance > 330 or game.camera.is_position_behind(enemy.position):
				continue
...
## Draw the title, campaign objective, controls, flight advice, and launch prompt.
...
## Draw the pause/results/defeat/victory overlay selected by the controller mode.
## This function only displays state; the controller handles confirmation and transitions.
func modal() -> void:
	...
	if game.mode == "pause":
		text(Vector2(376, 382), "TAB radar  ·  R upright  ·  M missile  ·  B bomb  ·  ESC resume", 16)
